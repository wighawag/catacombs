...
import {GameAccountData} from '$lib/account/account-data';
import {contractNetwork} from '../networks';

const logger = logs('connectiion');

export const accountData = new GameAccountData();
...
// But note that the game is designed to work without external RPC
			...
			if (!signature) {
				async function signMessage() {
					const msg = stringToHex(
						'Welcome to Game, Please sign this message only on trusted frontend. This gives access to your local data that you are supposed to keep secret.',
					);
