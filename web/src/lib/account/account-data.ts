...
function fromOnChainActionToPendingTransaction(
	hash: `0x${string}`,
	onchainAction: OnChainAction<GameMetadata>,
): PendingTransaction {
	return {
		hash,
...
export class GameAccountData extends BaseAccountHandler<AccountData, GameMetadata> {
...
function _filterOutOldActions(actions: OnChainActions<GameMetadata>): boolean {
	let changes = false;
