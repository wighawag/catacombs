...
export function computeEpoch(time: number) {
	...
	const COMMIT_PHASE_DURATION = Number(
		currentContractsInfos.contracts.Game.linkedData.commitPhaseDuration.slice(0, -1),
	);
	const REVEAL_PHASE_DURATION = Number(
		currentContractsInfos.contracts.Game.linkedData.revealPhaseDuration.slice(0, -1),
	);
	const START_TIME = 0;
	// --------------------
	// From Solidity
