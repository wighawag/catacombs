<script lang="ts">
	...
	$: isAdmin = false; // TODO  $account.address?.toLowerCase() === $contractsInfos.contracts.Game.linkedData.admin?.toLowerCase();
		...
		if (isActionPhase) {
			if (hash) {
				...
				await fetch(`${FUZD_URI}/api/internal/processQueue`)
					.then((v) => v.text())
					.then((v) => console.log(v));
				await increaseContractTime(parseInt($contractsInfos.contracts.Game.linkedData.revealPhaseDuration));
			} else {
			}
		}
	}
...
<div class="info" id="info-bar">
	...
	{:else if $connection.state !== 'Connected'}
		...
		{:else}
			<div>
				The game is a fully local client and we do not run any blockchain node for it. As such, it requires a wallet
	...
	{:else if $balance.state === 'Loaded'}
		{#if $balance.nativeBalance < parseEther('0.00001')}
