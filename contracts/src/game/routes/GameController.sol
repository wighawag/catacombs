...
contract GameController is Game {
    ...
    function join(uint256 characterID) external {
        Game.Store storage store = getStore();
        // 4 steps

        // 1. gather context (will be emitted)
        Context memory context = Context({characterID: characterID, sender: msg.sender});
        ...
        // 4. emit event
        emit Game.Joined(context.characterID, context.sender);
    }

    function _stateChanges(Context memory context) public pure returns (StateChanges memory stateChanges) {
        return StateChanges({characterID: context.characterID, controller: context.sender});
    }

    function _apply(Game.Store storage store, StateChanges memory stateChanges) internal {
        // TODO allow to set Basic for local key
        store.avatars[stateChanges.characterID].controllers[stateChanges.controller] = Game.ControllerType.Owner;
    }
}
